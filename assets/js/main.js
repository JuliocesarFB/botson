...
function updateTeamStats(players){
  ...
  const avg=arr=>(arr.reduce((s,v)=>s+v,0)/(arr.length||1)).toFixed(2);
...
function bindPlayerModal(){
  ...
  document.querySelectorAll(".player-card").forEach(card=>{
    card.addEventListener("click",()=>{
      ...
      const p=playersCache.find(pl=>pl.nickname?.toLowerCase().trim()===name);
      if(!p)return;
      document.getElementById("modalAvatar").src=p.avatar_url||"assets/imagens/default.png";
      ...
      document.getElementById("statWr").innerText=p.wr!==null?`${Number(p.wr).toFixed(1)}%`:"-";
      document.getElementById("statmatches").innerText=p.matches??"-";
      document.getElementById("statPresence").innerText=p.presence!==null?`${Number(p.presence).toFixed(1)}%`:"-";
...
function updateMVP(players){
  ...
  document.querySelectorAll(".player-card.mvp").forEach(c=>c.classList.remove("mvp"));
  const top=players.filter(p=>Number(p.kd||0)===maxKd);
  if(top.length!==1)return;
  const card=document.querySelector(`.player-card[data-player-name="${top[0].nickname}"]`);
